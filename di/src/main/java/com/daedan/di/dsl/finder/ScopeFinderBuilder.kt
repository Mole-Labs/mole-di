...
@ModuleBuilderDSL
class ScopeFinderBuilder : AbstractFinderBuilder() {
    ...
    inline fun <reified T : Any> findActivityRetainedScope(
        qualifier: Qualifier = TypeQualifier(T::class),
        noinline block: ActivityRetainedScopeFinderBuilder.() -> Finder = { Finder(qualifier) },
    ): Finder =
        Finder(
            qualifier = qualifier,
            next = block(ActivityRetainedScopeFinderBuilder()),
        )
}
